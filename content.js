...
function findPostContent(commentBoxNode) {
    ...
    const commonAncestorSelector2 = ".feed-shared-update-v2.feed-shared-update-v2--minimal-padding.full-height.relative.feed-shared-update-v2--e2e.artdeco-card";
    const postContentSelector = '.update-components-text.relative.update-components-update-v2__commentary'; // This should target the element that directly contains the text of the post.

    // Find the common ancestor element that contains the post content.
    const commonAncestor = commentBoxNode.closest(commonAncestorSelector) || commentBoxNode.closest(commonAncestorSelector2);
    if (!commonAncestor) {
